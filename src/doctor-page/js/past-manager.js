class PastManager {
    ...
    setRowData(species, breed, motivation, description, date, time, state, rowIndex, prenotationId) {
        let data = [species + ", " + breed, motivation, description, date + " alle " + time, state, this.getDetailsBtn(rowIndex, prenotationId), this.getResultBtn(rowIndex, prenotationId)];
    ...
    getResultBtn(rowIndex, prenotationId) {
        let input = document.createElement("input");
        input.setAttribute("type", "button");
        input.setAttribute("value", "Compila risultato");
